...
async def search_videos(
    ...
    """Search videos by title and/or category"""
    ...
    # Apply filters
    if query:
        stmt = stmt.filter(models.Video.title.ilike(f"%{query}%"))
    if category_id:
        stmt = stmt.filter(models.Video.category_id == category_id)
    ...
    # Convert to list of dicts with additional counts
    return [
        {
            "id": v.id,
            "title": v.title,
...
async def get_video_suggestions(db: AsyncSession, query: str, limit: int = 5):
    """Get video title suggestions for search autocomplete"""
    if not query:
        return []
    
    result = await db.execute(
        select(models.Video.title)
...
async def get_popular_search_terms(db: AsyncSession, limit: int = 5):
    """Get popular search terms (you might want to track searches separately)"""
    # This is a placeholder - you'd need a search history table for real implementation
    result = await db.execute(
        select(models.Video.title)
        .group_by(models.Video.title)
